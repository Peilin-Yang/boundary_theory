...
class SubqueriesLearning(RunSubqueries):
    ...
    def gen_clt(self, qid, _type=2):
        ...
        for subquery_id, subquery_str in subquery_mapping.items():
            ...
            for i, w in enumerate(withins):
                ...
                if np.any(np.isnan(centeroid_all)) or centeroid_all.shape[0] > 3 or centeroid_all.shape[0] < 2:
                ...
                for label_type in all_models[query_length]:
                    ...
                    if method == 1:
                        with open(os.path.join(model_root, model_fn)) as f:
                            model = f.readlines()[-1]
                        feature_weights = [(int(ele.split(':')[0]), math.fabs(float(ele.split(':')[1]))) for ele in model.split()[1:-1]]
                    elif method == 2:
    ...
    def gen_resources_for_crowdsourcing_atom(self, qid, optimal_subquery_id, ap_diff):
        ...
        if not os.path.exists(results_root):
            os.makedirs(results_root)
        self.dump_doc(os.path.join(self.output_root, 'runfiles_allterms', optimal_subquery_runfile), 
            rel_docs, results_root, True)
        self.dump_doc(os.path.join(self.output_root, 'runfiles_allterms', allterm_subquery_runfile), 
