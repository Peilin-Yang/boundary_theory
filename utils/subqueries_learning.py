    ...
    def cross_testing(train, test, query_length=2, _type='int', method='lambdamart'):
        ...
        if method == 'lambdamart':
            ...
            if not os.path.exists(model_output_fn):
                command = 'java -jar ~/Downloads/RankLib-2.8.jar -train %s -ranker 6 -save %s'.split() % (trainging_fn, model_output_fn)
            ...
            if not os.path.exists(predict_fn):
                command = 'java -jar ~/Downloads/RankLib-2.8.jar -load %s -rank %s -score %s'.split() 
