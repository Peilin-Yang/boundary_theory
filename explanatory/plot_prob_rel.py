                    ...
                    for j in range(1, FittingModels().size()+1):
                        ...
                        if not fitting is None:
                            ...
                            if re.search(r'^tf\d+$', _method):
                                print [x_dict[x][1] for x in raw_xaxis], fitting[0], fitting[2], sum_yaxis, FittingModels().curve_fit_mapping(fitting[0])([x_dict[x][1] for x in raw_xaxis], *fitting[2])*sum_yaxis
